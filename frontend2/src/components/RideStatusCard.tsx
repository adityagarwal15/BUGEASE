...
const RideStatusCard: React.FC<RideStatusCardProps> = ({ ride, onCancel }) => {
  ...
  return (
    <Card className="glass-card overflow-hidden border-t-4 animate-fade-in" style={{borderTopColor: 'rgb(139, 92, 246)'}}>
      ...
      <CardContent className="pb-4">
        <div className="space-y-4">
          ...
          <div className="flex items-start space-x-3 text-sm">
            ...
            <div>
              <span className="text-muted-foreground">From:</span> {ride.pickupLocation.name}
            </div>
          </div>
          ...
          <div className="flex items-start space-x-3 text-sm">
            ...
            <div>
              <span className="text-muted-foreground">To:</span> {ride.dropoffLocation.name}
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3 text-sm">
            ...
            <div>
              <span className="text-muted-foreground">Request time:</span> {formatTime(ride.requestTime)}
            </div>
          </div>
          ...
          {ride.estimatedArrival && (
            <div className="flex items-center space-x-3 text-sm">
              ...
              <div>
                <span className="text-muted-foreground">Estimated arrival:</span> {formatTime(ride.estimatedArrival)}
              </div>
            </div>
          )}
          ...
          {ride.estimatedDuration && (
            <div className="mt-2 bg-muted p-3 rounded-md text-center">
              <span className="text-muted-foreground">Estimated journey time:</span>
              <div className="text-xl font-semibold mt-1">{ride.estimatedDuration} mins</div>
            </div>
          )}
        </div>
      </CardContent>
