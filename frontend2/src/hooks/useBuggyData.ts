...
import { trackingService, Buggy, BuggyLocation, LocationHistory } from '@/services/trackingService';
...
export const useBuggyData = () => {
  ...
  const fetchLocationHistory = async (buggyId: number, since = '1h'): Promise<LocationHistory[]> => {
    setIsLoading(true);
    setError(null);
  ...
  // Update buggy status
  const updateBuggyStatus = async (buggyId: number, isRunning: boolean): Promise<Buggy | null> => {
    setIsLoading(true);
    setError(null);
    
    try {
      const updatedBuggy = await trackingService.updateBuggyStatus(buggyId, isRunning);
