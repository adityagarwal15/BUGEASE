...
const simulateMovement = (location: DriverLocation): DriverLocation => {
  ...
  if ((location.status === 'available' || location.status === 'busy') && location.speed > 0) {
...
export const useDriverLocations = (updateInterval = 1000) => {
  const [locations, setLocations] = useState<DriverLocation[]>(initialDriverLocations);
...
export const getDriverLocationById = (id: string, locations: DriverLocation[]) => {
  return locations.find(location => location.driverId === id || location.buggyId === id);
};
