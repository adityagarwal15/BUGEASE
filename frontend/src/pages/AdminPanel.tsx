...
const AdminPanel = () => {
  ...
  // Get buggy stats
  const availableBuggies = mockBuggies.filter(
    b => b.status === "available"
  ).length;
  const busyBuggies = mockBuggies.filter(b => b.status === "busy").length;
  const maintenanceBuggies = mockBuggies.filter(
  ...
  // Get ride stats
  const completedRides = mockRideHistory.filter(
    r => r.status === "completed"
  ).length;
  const cancelledRides = mockRideHistory.filter(
                  ...
                  <div className="mt-4 space-y-2">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-medium">
                        {availableBuggies + busyBuggies} / {totalBuggies}
                      </span>
                    </div>
                    <Progress
                      value={
                        ((availableBuggies + busyBuggies) / totalBuggies) * 100
                      }
                      className="h-1"
                    />
                  </div>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="mt-4 space-y-2">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-medium">
                        {Math.round((availableBuggies / totalBuggies) * 100)}%
                      </span>
                    </div>
                    <Progress
                      value={(availableBuggies / totalBuggies) * 100}
                      className="h-1"
                    />
                  </div>
                </CardContent>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="mt-4 space-y-2">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-medium">
                        {Math.round((maintenanceBuggies / totalBuggies) * 100)}%
                      </span>
                    </div>
                    <Progress
                      value={(maintenanceBuggies / totalBuggies) * 100}
                      className="h-1"
                    />
                  </div>
                </CardContent>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="mt-4 space-y-2">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-medium">
                        {Math.round(
                          (completedRides / (completedRides + cancelledRides)) *
                            100
                        )}
                        %
                      </span>
          ...
          <TabsContent value="buggies" className="space-y-6">
            <Card className="glass-card">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                ...
                <Button size="sm">Add New Buggy</Button>
              </CardHeader>
              <CardContent>
                      ...
                      {mockBuggies.map(buggy => (
                        <tr key={buggy.id} className="hover:bg-muted/50">
                          ...
                          <td className="py-3 px-4">
                            <div className="flex items-center">
                              <span
                                className={getBatteryColor(buggy.batteryLevel)}
                              >
                                {buggy.batteryLevel}%
                              </span>
                              <div className="ml-2 h-1.5 w-12 bg-muted rounded-full overflow-hidden">
                                <div
                                  className={cn(
                                    "h-full rounded-full",
                                    buggy.batteryLevel > 70
                                      ? "bg-green-500"
                                      : buggy.batteryLevel > 30
                                        ? "bg-amber-500"
                                        : "bg-red-500"
                                  )}
                          ...
                          <td className="py-3 px-4">
                            <div className="flex items-center">
                              ...
                              <span>
                                {buggy.lastUpdated.toLocaleTimeString([], {
                                  hour: "2-digit",
                                  minute: "2-digit",
                                })}
                              </span>
                          ...
                          <td className="py-3 px-4 text-right">
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                ...
                                <DropdownMenuItem
                                  onClick={() =>
                                    handleActionClick("View", buggy.id)
                                  }
                                >
                                  View Details
                                </DropdownMenuItem>
                                <DropdownMenuItem
                                  onClick={() =>
                                    handleActionClick("Track", buggy.id)
                                  }
                                >
                                  Track Location
                                </DropdownMenuItem>
                                <DropdownMenuItem
                                  onClick={() =>
                                    handleActionClick("Assign", buggy.id)
                                  }
                                >
                                  Assign Driver
                                </DropdownMenuItem>
                                <DropdownMenuItem
                                  onClick={() =>
                                    handleActionClick("Maintenance", buggy.id)
                                  }
                                >
                ...
                <div className="flex items-center justify-center h-48 bg-muted rounded-md">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-muted-foreground max-w-md mx-auto">
                      This section would display a list of drivers, their
                      assigned buggies, schedules, and performance metrics.
                    </p>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-center h-48 bg-muted rounded-md">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-muted-foreground max-w-md mx-auto">
                      This section would display current and pending ride
                      requests, allowing admins to manage and prioritize rides.
                    </p>
                  </div>
                </div>
              </CardContent>
