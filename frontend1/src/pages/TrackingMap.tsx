...
import { useWebSocketLocations } from '@/services/websocket/locationWebSocketService';
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="mb-6">
          ...
          <p className="text-white/70 mt-1">
            ...
            {isConnected && <span className="text-[#00d1ff] ml-2">• Live</span>}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="hidden lg:block lg:col-span-3 space-y-6">
              <Collapsible open={isRidePanelOpen} onOpenChange={setIsRidePanelOpen}>
                <CollapsibleTrigger asChild>
                  <Button 
                    variant="ghost" 
                    className="w-full flex justify-between items-center mb-2 bg-[#1a1a2e]/50 backdrop-blur-md border border-white/5 text-white hover:bg-[#1a1a2e]/80"
                  >
          ...
          <div className={cn(
            ...
            <Card className="glass-panel h-[calc(100vh-200px)] md:h-[600px] w-full relative overflow-hidden border-[#7f5af0]/20 bg-[#1a1a2e]/70 backdrop-blur-md">
              ...
              {isMobile && (
                <Button
                  ...
                  className="absolute top-4 left-4 z-30 opacity-80 hover:opacity-100 bg-[#7f5af0] hover:bg-[#7f5af0]/80"
                >
              ...
              <div className="absolute bottom-4 left-4 z-30 bg-[#1a1a2e]/80 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-medium">
                {isConnected ? (
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-white/90">Static Data</span>
                  </div>
                )}
              </div>
              ...
              <div className={`absolute top-4 right-4 transition-all duration-300 ${showControls ? 'translate-x-0' : 'translate-x-full'}`}>
                <Button
                  size="sm"
                  variant="outline"
                  className="absolute -left-12 bg-[#1a1a2e]/80 backdrop-blur-sm border border-white/10 shadow-md text-white hover:bg-[#1a1a2e]/60"
                  onClick={() => setShowControls(!showControls)}
                >
                  {showControls ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </Button>
                
                <Card className="glass-panel w-[220px] shadow-lg bg-[#1a1a2e]/80 backdrop-blur-md border border-white/10">
                  <CardHeader className="p-3 pb-1.5">
                    <CardTitle className="text-sm flex items-center gap-2 text-white">
                      <Layers className="h-4 w-4 text-[#00d1ff]" /> Map Layers
                    </CardTitle>
                  </CardHeader>
                    ...
                    {mapLayers.map(layer => (
                      <Button
                        ...
                        className={`w-full justify-start text-xs h-8 ${layer.disabled ? 'opacity-50 cursor-not-allowed' : ''} 
                          ${layer.active ? 'bg-[#7f5af0] hover:bg-[#7f5af0]/80 text-white' : 'text-white/70 border-white/10 hover:bg-white/5'}`}
                        onClick={() => toggleLayer(layer.id)}
                        disabled={layer.disabled}
                      >
                        <layer.icon className="mr-2 h-3.5 w-3.5" />
                        {layer.name}
                        {layer.disabled && <span className="ml-auto text-xs text-white/40">(Disabled)</span>}
                      </Button>
                    ))}
                  </CardContent>
                </Card>
              ...
              {!isMobile && (
                <Button
                  ...
                  variant="outline"
                  onClick={() => setShowSidePanels(!showSidePanels)}
                  className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-[#1a1a2e]/80 text-white border-white/10 backdrop-blur-sm hover:bg-[#1a1a2e]/60"
                >
          ...
          <div className={cn(
            ...
            <Card className="glass-panel sticky top-24 bg-[#1a1a2e]/70 backdrop-blur-md border border-white/10">
              <CardHeader className="p-4 pb-2 flex flex-row items-center justify-between">
                <CardTitle className="text-sm text-white">Available Buggies</CardTitle>
                <Link to="/book" className="text-xs text-[#00d1ff] hover:text-[#00d1ff]/80 hover:underline flex items-center">
                  Book a Ride
                </Link>
              </CardHeader>
              <CardContent className="p-4 pt-2 max-h-[500px] md:max-h-[550px] overflow-y-auto">
                {isLoading ? (
                  ...
                  <div className="py-8 text-center">
                    ...
                    <p className="text-white/50">No buggies available</p>
                    <p className="text-xs text-white/30 mt-1">Check back later or refresh</p>
                  </div>
                )}
              </CardContent>
            </Card>
