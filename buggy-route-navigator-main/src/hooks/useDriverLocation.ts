...
// Base WebSocket URL
const WS_BASE_URL = 'ws://127.0.0.1:8000/ws'; // Updated to use actual WebSocket URL
...
export const useDriverLocation = (buggy: BuggyDetails | null, isRunning: boolean) => {
  ...
  useEffect(() => {
    ...
    // Create WebSocket connection with updated URL
    const socket = new WebSocket(`${WS_BASE_URL}/location/updates/?token=${token}`);
